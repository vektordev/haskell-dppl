...
Batched output is tested by the `BatchedPython` group (`End2EndTesting.batchedPythonTests`): every eligible corpus program's batched code is run over a batch of its query points in a **real torch interpreter** and compared per-element to the point's expected `.tst` value (as of M3: **92 programs, 461 points** — M3's `IRError` poison admission added the extra program). A second property, `gradients-nan-free` (M3 acceptance), takes each eligible non-neural program whose code contains a `safe_log`/`safe_div` (21 programs — the `logNormal` family and friends), runs its `forward` on a boundary-straddling batch with `requires_grad` on the sample, back-propagates, and asserts the sample gradient is NaN/Inf-free. It needs a torch-enabled Python — found via `NEST_TORCH_PYTHON` → a venv path → `python3` — and **skips with a visible note** if none imports torch, so a torch-less CI stays green. The driver runs from a temp file (deep neural logit literals exceed the `python -c` arg-length limit); it admits neural programs regardless of their `.tst` Python-routing header, batching each per-point `(2, [logits])` symbol into a `[B, n]` tensor and installing an identity mock (mode-2 verbatim logits) for every declared network. topK/marginal queries and batched `generate` are later milestones.
