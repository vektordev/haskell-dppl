...
- **The batched backend** (`SPLL.CodeGenPyTorchBatched.generateFunctionsBatched`, M2a numeric + M2b neural) emits `torch.where` for selects and tensor formulas for everything else, over the **tensor fragment** only: float/int/bool leaves in fixed-shape tuples (structure-of-arrays: tuple structure stays a Python `T` object whose leaves are `[B]` tensors, so `dim`, the impossibility flag, and the branch count are per-element tensors too). **M2b** adds neural/enumerable programs: `IRApply` (network calls + cross-function decoder calls, reusing the scalar `_prob`→`.forward` LUT), `IRIndex` (a constant logit slice `out[..., i]` or a data-dependent gather `nn_gather(out, idx)`), `IREnumSum` (unrolled inline over the resolved `MultiValue`'s enum axis), and `IRIsPossible` (scalar-discrete membership → an `is_member` `[B]` mask) — reaching `mNistAdd`-style MNIST-addition inference. A **call-graph guard** (`checkCallGraph`) still refuses recursion (a cycle in the prob/integ call graph, e.g. `dice`) and calls reaching a non-emitted method (`generate`/`normal_params`, e.g. `factorial`/`flip`) — the refusals the old blanket `IRApply` refusal gave for free. **M3** adds **gradient hygiene**: every `OpLog`/`OpDiv` is routed through the double-`where` helpers `safe_log`/`safe_div` (mask the op's input to its safe domain so the local derivative stays finite; the enclosing `torch.where` discards the harmless value), because otherwise autograd flows `0·inf = NaN` back through the untaken (log-of-non-positive / divide-by-zero) arm of a select even though the forward *value* is correct — a defect the value differential can't see (`logNormal` is the canonical case). On in-domain elements the result is bit-identical to the plain op, so no expected value changes. `IRError` refusal arms are now emittable as a NaN `poison()` constant that the enclosing select masks away (a NaN *constant* has no gradient path, so it never itself NaNs a gradient; a surviving poison surfaces as NaN in the value differential — the poison-debug check without a separate flag), which admits set-witness programs carrying an `IRError` arm (e.g. `letProbIntervalPair`) into the fragment. The density/CDF twins need no masking (`exp`/`erf`/`clamp`, all grad-safe). Otherwise-outside-fragment programs (lists, `Either`, `IRLambda`, `VAny`/`VAnyExcept` marginal constants, composite `IRIsPossible`) are refused with a diagnostic naming the offending construct. `VAny`/query-type-guard handling is stripped at the batched entry (`prepBatchedBody`), not represented. Runtime lib: `pythonLibBatched.py` (torch density/CDF twins, elementwise `T.__eq__`, `asmask`, M2b's `nn_gather`/`is_member`, plus M3's `safe_log`/`safe_div`/`poison`).

Batched output is tested by the `BatchedPython` group (`End2EndTesting.batchedPythonTests`): every eligible corpus program's batched code is run over a batch of its query points in a **real torch interpreter** and compared per-element to the point's expected `.tst` value (as of M3: **92 programs, 461 points** — M3's `IRError` poison admission added the extra program). A second property, `gradients-nan-free` (M3 acceptance), takes each eligible non-neural program whose code contains a `safe_log`/`safe_div` (21 programs — the `logNormal` family and friends), runs its `forward` on a boundary-straddling batch with `requires_grad` on the sample, back-propagates, and asserts the sample gradient is NaN/Inf-free. It needs a torch-enabled Python — found via `NEST_TORCH_PYTHON` → a venv path → `python3` — and **skips with a visible note** if none imports torch, so a torch-less CI stays green. The driver runs from a temp file (deep neural logit literals exceed the `python -c` arg-length limit); it admits neural programs regardless of their `.tst` Python-routing header, batching each per-point `(2, [logits])` symbol into a `[B, n]` tensor and installing an identity mock (mode-2 verbatim logits) for every declared network. topK/marginal queries are later milestones.

**M4** adds **batched generate** (`rand(n)`/`randn(n)` draw a whole `[n]`-shaped batch; a random `if` in generate mode renders exactly like a prob/integ select — `torch.where`, both arms drawing independently for the whole batch, so each element gets one arm's fresh draw and the result is the same mixture distribution as scalar generate, just with harmless extra randomness for the untaken arm). Scoped to the non-neural tensor fragment only (matching M2a); a neural decoder group's own generate (categorical/Gaussian decoder sampling) is out of scope and gets a runtime-raising stub instead — see below — with neural generate parity filed as its own follow-on task. Generate ineligibility is **per-class and best-effort**, unlike forward/integrate: `generateFunctionsBatched` stays a single hard-`Either` pipeline for forward/integrate, but a group's generate independently either compiles or degrades to `raise NotImplementedError(...)` (accepting `*args, **kwargs` so a caller threading the batch-size parameter through a stubbed callee doesn't itself fail on arity first) — necessary because every neural declaration's decoder group unconditionally has a generate function, so a hard whole-program failure on generate ineligibility would have broken batched compilation of *every* neural corpus program the moment generate started being attempted at all. Three shapes degrade to a stub, each named in the diagnostic: a neural decoder group (`SPLL.AutoNeural.isNeuralDecoderGroup`, the `_auto` group-name suffix convention, checked structurally rather than by trying and failing the fragment guard since `IRApply` is broadly emittable already); a cycle in the generate-only call graph (`hasGenCycle` — unlike prob/integ's recursion refusal, which is purely a compile-time concern, unbounded generate recursion would make Python actually stack-overflow at runtime under both-arm-eager select semantics); or any other non-fragment construct (the existing `batchedGuard`, now with `IRSample{}` admitted as fragment content). The batch-size parameter is a reserved-looking internal name (`_batchN`, matching the `_r0`/`cse_0` convention) rather than something public-facing like `n`, specifically to avoid colliding with a genuinely `n`-parameterised SPLL function. Tested by a third `BatchedPython` property, `generate-density-matches-expected`: for every eligible non-neural program, a large fixed-seed batch from `main.generate(_batchN)` is checked against the program's *existing* prob query points via an epsilon-window empirical-density estimate (mirroring `Spec.hs`'s `testSamplingProb` idiom) compared to the `.tst`-declared density within `samplingTolerance` — reusing corpus ground truth rather than needing a second sampling distribution to compare against (**78 of the 92 differential programs** — the rest are neural, out of scope, or need more than the batch-size parameter, e.g. a `ThetaTree` test override).
