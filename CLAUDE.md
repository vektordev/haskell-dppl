...
### The sum-type / MAR showcase series

`testCases/showcase_*.ppl` is a deliberately redundant, deliberately *ordered* corpus (design sum-type-showcase): fourteen programs that climb from `main = left 1` to a three-sensor neural product of experts, each one adding exactly one capability. Several duplicate an existing corpus program's shape (`showcase_either_const` vs `either_const`); that is the point — the series doubles as the specification of what sum types and structural marginalization are supposed to do, so it reads top-to-bottom and graduates a level at a time as the compiler gains capability. Levels 0-4 all pass today. `test/TestInternals.hs`'s `sumTypeShowcaseTests` pins the *identities* behind the frozen `.tst` decimals — `p(Just k) = P_cam(k)·P_depth(k)` against the mock networks' own class probabilities, the triple product, posterior renormalization by `p(Just ANY)`, and `p(Left ANY) + p(Right ANY) = 1` across the whole series.

Two things the series is worth reading for:

- **`observe` in userspace.** There is no `observe` keyword on this branch; the series writes out the idiom it desugars to, `let v = base in if pred v then right v else left ()`, with `Just x = right x` and `Nothing = left ()`. The base **must** be let-bound (it occurs twice; a probabilistic base spliced in twice is two independent draws). Two forms that look equivalent do not work: a polymorphic higher-order `observe base pred` refuses (the predicate stays under an unreduced `Apply`/`Lambda`, which defeats both point inversion and the set-valued witness fallback), and a let-bound *lambda* predicate works for a discrete base but hits the same wall for a continuous one. Only the fully-inlined predicate compiles across the board.
- **Fusion is a product, not a mixture.** `showcase_poe_*` fuse sensors by *observing their agreement* — `let v = camNN img in let w = depthNN depth in if v == w then right v else left ()` — which enumerates to `p(Just k) = P_cam(k)·P_depth(k)`, exactly (verified to 1e-9, not to `probTolerance`). Both experts must be **let-bound**: written inline as `if camNN img == depthNN depth`, the right-hand side compiles through the *generator* instead and the prob function dies with `Uniform Gen is not det`.

`showcase_observe_inequality` (userspace `observe` over a continuous base — a truncated Normal) is the one program in the series with a missing line: `p(Right ANY)`, the `P(Just ANY) = 0.5` denominator, hard-errors (`less than can only compare two numbers: (VAny, VFloat 0.0)`) because the set-witness path does not handle `ANY` inside a constructor slot of a comparison target. It is also the series' only entry in `Spec.hs`'s `logSpaceUncoveredPrograms`, for the ordinary reason (it reaches the set-witness machinery, which is linear-only).
