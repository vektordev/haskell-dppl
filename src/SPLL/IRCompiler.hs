...
toIRIntegrateSave conf typeEnv (Lambda _ n e) lower higher = do
  integ <- lift $ return $ generateLetInBlock conf (runWriterT (toIRIntegrateSave conf typeEnv e lower higher))
