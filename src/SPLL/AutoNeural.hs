...
makeAutoNeural (name, (TArrow TSymbol target), tag) =
  [(name ++ "_" ++ show (getSize plan) ++ "_gen" , IRLambda symbol $ makeGen  plan name),
   (name ++ "_" ++ show (getSize plan) ++ "_prob", IRLambda symbol $ makeProb plan name)]
...
makeProb plan nn_name = IRLambda "sample" $ IRLetIn vector (IREvalNN nn_name (IRVar "l_x_neural_in")) (IRTCons m (IRTCons dim bc))
  where (m, dim, bc) = (makeProbRec plan 0 (IRVar "sample"))
...
test = do
  let irdefs = makeAutoNeural ("readMNist", TArrow TSymbol TInt, Just $ EnumRange ((VInt 0), (VInt 9)))
  putStrLn (pPrintIREnv irdefs)

test2 = do
  let irdefs = makeAutoNeural ("regressFloat", TArrow TSymbol TFloat, Nothing)
  putStrLn (pPrintIREnv irdefs)
...
test4 = do
  let irdefs = makeAutoNeural ("tuple", TArrow TSymbol (Tuple TInt TInt), Just $ EnumRange ((VTuple (VInt 7) (VInt 3)), (VTuple (VInt 9) (VInt 5))))
  putStrLn (pPrintIREnv irdefs)

test5 = do
  let irdefs = makeAutoNeural ("tuple", TArrow TSymbol (Tuple TInt TInt), Just $ EnumList [(VTuple (VInt 7) (VInt 3)), (VTuple (VInt 9) (VInt 5))])
  putStrLn (pPrintIREnv irdefs)
