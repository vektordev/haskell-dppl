...
-- "neural writeLogits :: T of M" registers a standalone PartitionPlan annotation for T in
-- Program.writeLogitsDecls (see SPLL.Lang.Types.writeLogitsDecls), without declaring a callable
-- neural network -- "writeLogits" is a reserved name and contributes nothing to 'neurals'.
prop_neuralWriteLogitsRegistersPartitionPlan :: Property
...
-- MultiValue into Program.writeLogitsDecls -- this is what lets a later read-logits
-- network for the same RType, declared without its own "of" clause, pick up the same
-- PartitionPlan.
